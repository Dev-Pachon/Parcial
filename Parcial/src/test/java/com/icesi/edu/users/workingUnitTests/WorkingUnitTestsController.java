...
public class WorkingUnitTestsController {
    ...
    private DocumentDTO createTestDocument(String name, DocumentStatus documentStatus) {
        return DocumentDTO.builder().name(name).documentId(DOCUMENT_UUID).status(documentStatus).text("random").priority(1).build();
    }
    @Test
    public void onlyLettersNSpacesOnName() {
        try {
            documentController.createDocument(createTestDocument("$%&/Fail)", DocumentStatus.CREATED));

        } catch (DocumentException e) {
            assertEquals(HttpStatus.BAD_REQUEST, e.getHttpStatus());
            assertNotNull(e.getError());
            DocumentError error = e.getError();
            assertEquals("Only letters and spaces are allowed", error.getMessage());
