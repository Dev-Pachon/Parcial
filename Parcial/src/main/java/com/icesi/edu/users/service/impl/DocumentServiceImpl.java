...
public class DocumentServiceImpl implements DocumentService {
    ...
    public Document updateDocument(Document document) {
        documentNotApproved(document.getStatus());
        return documentRepository.save(document);
    }

    private void documentNotApproved(DocumentStatus state) {
        if (state.equals(DocumentStatus.APPROVED))
            throw new DocumentException(HttpStatus.METHOD_NOT_ALLOWED, new DocumentError(DocumentErrorCode.CODE_02, DocumentErrorCode.CODE_02.getMessage()));
    }
}
