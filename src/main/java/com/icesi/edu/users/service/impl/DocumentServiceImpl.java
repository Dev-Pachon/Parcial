...
public class DocumentServiceImpl implements DocumentService {
    ...
    private void exceptionIsThrownWhenTryingToUpdateDocumentWithStatusApproved(Document document) {
    	Optional<Document> oldDocument = documentRepository.findById(document.getDocumentId());
        if(oldDocument.isPresent()){
            Document doc = oldDocument.get();
        	DocumentStatus status = doc.getStatus();
        	if(DocumentStatus.APPROVED.getStatus().equals(status.getStatus())) {
        		throw new DocumentException(HttpStatus.BAD_REQUEST, new DocumentError(DocumentErrorCode.CODE_03.name(), DocumentErrorCode.CODE_03.getMessage()));
        	}
        }
    }
}
